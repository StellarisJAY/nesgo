...
func (b *Bus) ReadMemUint8(addr uint16) byte {
	...
	case addr >= 0x6000 && addr <= 0x7fff:
		return b.batteryBackedRAM[addr-0x6000]
...
func (b *Bus) WriteMemUint8(addr uint16, val byte) {
	...
	case addr == 0x4017:
		//skip joyPad 2
	case addr >= 0x6000 && addr <= 0x7fff:
		b.batteryBackedRAM[addr-0x6000] = val
	case addr >= ROMStart:
