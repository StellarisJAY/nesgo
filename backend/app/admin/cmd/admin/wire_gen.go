...
func wireApp(confServer *conf.Server, confData *conf.Data, registry *conf.Registry, auth *conf.Auth, logger log.Logger) (*kratos.App, func(), error) {
	...
	gamingUseCase := biz.NewGamingUseCase(gamingRepo, roomRepo, userRepo, logger)
	adminService := service.NewAdminService(gameFileUseCase, adminUseCase, roomUseCase, gamingUseCase, logger)
